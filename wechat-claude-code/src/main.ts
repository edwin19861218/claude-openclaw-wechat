...
async function handleMessage(
  ...
  const routingMode: RoutingMode = session.routingMode ?? 'claude';
...
async function sendToOpenClaw(
  ...
  try {
    ...
    if (result.ok && result.reply?.text) {
      sessionStore.addChatMessage(session, 'assistant', result.reply.text);
      const chunks = splitMessage(result.reply.text);
      for (const chunk of chunks) {
        await sender.sendText(fromUserId, contextToken, chunk);
      }
    } else if (result.error) {
      await sender.sendText(fromUserId, contextToken, `⚠️ OpenClaw 处理失败: ${result.error}`);
    } else {
      await sender.sendText(fromUserId, contextToken, 'ℹ️ OpenClaw 无返回内容');
    }
  } catch (err) {
    const errorMsg = err instanceof Error ? err.message : String(err);
    logger.error('Error in sendToOpenClaw', { error: errorMsg });
    await sender.sendText(fromUserId, contextToken, '⚠️ OpenClaw 请求失败，请确认 bridge 和 gateway 已启动。');
  } finally {
    session.state = 'idle';
    sessionStore.save(account.accountId, session);
  }
